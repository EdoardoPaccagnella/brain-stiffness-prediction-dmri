...
def add_coord_channels(tensor):
    """
    Adds 3 channels with normalized coordinates (Z, Y, X) to a 3D tensor [C, D, H, W].
    """
    ...
    z = torch.linspace(0, 1, D, device=device, dtype=dtype).view(1, D, 1, 1).expand(1, D, H, W)
    y = torch.linspace(0, 1, H, device=device, dtype=dtype).view(1, 1, H, 1).expand(1, D, H, W)
    x = torch.linspace(0, 1, W, device=device, dtype=dtype).view(1, 1, 1, W).expand(1, D, H, W)

    coord_tensor = torch.cat([z, y, x], dim=0)  # [3, D, H, W]
    return torch.cat([tensor, coord_tensor], dim=0)  # [C+3, D, H, W]
    
def extract_patches(img, patch_size=(32, 32, 32), stride=(16, 16, 16)):
    """
    Divide a 3D volume (C, D, H, W) into a list of patches.
    """
    assert img.ndim == 4  # [C, D, H, W]
    c, d, h, w = img.shape
    slices = dense_patch_slices(patch_size=patch_size, image_size=(d, h, w), scan_interval=stride)
    patches = [img[:, s[0], s[1], s[2]] for s in slices]
    return patches, slices
    
# Custom dataset for loading NIfTI images
class NiftiDataset(Dataset):
    def __init__(self, target_img_paths, input_img_paths, target_shape=(64,64,64), patch_size=(32, 32, 32), stride=(16, 16, 16), cache=True):
        ...
        if self.cache:
            print("Caching images into memory as patches...")
            for subj_idx, (t_path, i_paths) in enumerate(zip(self.target_img_paths, self.input_img_paths)):
                # Target 
                t_img = nib.load(t_path).get_fdata(dtype=np.float32)
                t_tensor = torch.tensor(skimage.transform.resize(t_img, self.target_shape, mode='constant'), dtype=torch.float32).unsqueeze(0)
                ...
                input_tensor = torch.tensor(np.stack(imgs, axis=0), dtype=torch.float32)  # [C, D, H, W]
                input_tensor = add_coord_channels(input_tensor)

                # Extract patches
                patches_input, slices = extract_patches(input_tensor, patch_size=self.patch_size, stride=self.stride)
                patches_target, _ = extract_patches(t_tensor, patch_size=self.patch_size, stride=self.stride)

                self.input_patches.extend(patches_input)
                self.target_patches.extend(patches_target)
                self.patch_locations.extend([(subj_idx, s) for s in slices])

            print(f"Done caching {len(self.input_patches)} patches.")

    def __len__(self):
        return len(self.input_patches)

    def __getitem__(self, idx):
        return self.input_patches[idx], self.target_patches[idx]

in_chs = 4 #choose the number of parameters that you want to use + 3 for the coordinates
...
all_parameters = ['T1_t1_to_MNI_normalizedxparam.nii.gz']
...
for SubjectName in ListofSubjectNames:
    ...
    relative_pathnames = ['T1_t1_to_MNI_normalizedxparam.nii.gz']
...
in_chs = len(relative_pathnames)+3
print(in_chs)
...
train_dataset = NiftiDataset(train_targets, train_inputs, target_shape=target_shape, patch_size=patch_size, stride=stride, cache=True)
val_dataset   = NiftiDataset(val_targets, val_inputs, target_shape=target_shape, patch_size=patch_size, stride=stride, cache=True)
test_dataset  = NiftiDataset(test_targets, test_inputs, target_shape=target_shape, patch_size=patch_size, stride=stride, cache=True)
...
experiment_name = f'T1_patchwise32_stride16_justnorm_kern5'
...
# Save NIfTI predictions directory
...
inferer = SlidingWindowInferer(
    roi_size=(32, 32, 32),  # same patch size of training
    sw_batch_size=1,
    overlap=0.5,
    mode='gaussian'
)
...
with torch.no_grad():
    for subj_idx, (input_paths, target_path) in enumerate(zip(test_inputs, test_targets)):
        # === Load input ===
        imgs = [nib.load(p).get_fdata(dtype=np.float32) for p in input_paths]
        input_volume = np.stack(imgs, axis=0)  # [C, D, H, W]
        input_tensor = torch.tensor(input_volume, dtype=torch.float32).to(device)  # [C, D, H, W]

        # === Add spatial locations ===
        input_tensor = add_coord_channels(input_tensor)  # [C+3, D, H, W]
        input_tensor = input_tensor.unsqueeze(0)  # [1, C+3, D, H, W]

        # === Prediction sliding window ===
        pred = inferer(inputs=input_tensor, network=model)  # [1, 1, D, H, W]

        # === Save NIfTI ===
        patient_name = os.path.basename(os.path.dirname(target_path))
        output_filename = os.path.join(nifti_output_dir, f"{patient_name}_pred.nii.gz")
        save_prediction_as_nifti(prediction=pred, reference_nifti_path=target_path, output_filename=output_filename)
...
def ssim_mask(pred, target, window_size=11, size_average=True, mask=None):
    if pred.dim() == 4:
        (_, channel, height, width) = pred.size()
        conv = F.conv2d
    elif pred.dim() == 5:
    ...
    def create_window(window_size, channel):
        ...
        if pred.dim() == 4:
            ...
            window_3d = gauss.unsqueeze(1) * gauss.unsqueeze(0)
            window_3d = window_3d.unsqueeze(2) * gauss.unsqueeze(0).unsqueeze(0).unsqueeze(2)
            return window_3d.unsqueeze(0).repeat(channel, 1, 1, 1, 1)
    ...
    if mask is not None:
        ...
        if size_average:
            return 1 - (ssim_map * mask).sum() / mask.sum()
        else:
...
with torch.no_grad():
    for subj_idx, (input_paths, target_path) in enumerate(zip(test_inputs, test_targets)):
        patient_name = os.path.basename(os.path.dirname(target_path))
        print(f"Valutazione soggetto: {patient_name}")

        target_nii = nib.load(target_path)
        target_np = target_nii.get_fdata().astype(np.float32)
        target_tensor = torch.from_numpy(target_np).unsqueeze(0).unsqueeze(0).to(device)  # [1, 1, D, H, W]

        input_vols = []
        for path in input_paths:
            img = nib.load(path).get_fdata().astype(np.float32)
            input_vols.append(img)
        input_np = np.stack(input_vols, axis=0)  # [C, D, H, W]
        
        input_tensor = torch.from_numpy(input_np).to(torch.float32).to(device)  # [C, D, H, W]
        input_tensor = add_coord_channels(input_tensor)  # [C+3, D, H, W]
        input_tensor = input_tensor.unsqueeze(0)  # [1, C+3, D, H, W]

        mask_tensor = (target_tensor > 0).float()  # [1, 1, D, H, W]

        # === Prediction ===
        output_tensor = model(input_tensor)  # [1, 1, D, H, W]

        # === Apply the mask ===
        valid_outputs = output_tensor[mask_tensor.bool()]
        valid_targets = target_tensor[mask_tensor.bool()]

        if valid_outputs.numel() == 0:
            print(f"[!] Maschera vuota per {patient_name}, salto.")
            continue

        mse_value = F.mse_loss(valid_outputs, valid_targets).item()
        ssim_value = 1 - ssim_mask(output_tensor, target_tensor, mask=mask_tensor).item()
        psnr_value = calculate_psnr(mse_value, max_val)
...
print(f"\n[Test] MSE: {mean_mse:.4f} ± {std_mse:.4f}, SSIM: {mean_ssim:.4f} ± {std_ssim:.4f}, PSNR: {mean_psnr:.2f} ± {std_psnr:.2f} dB")
...
pd.DataFrame(results).to_csv(os.path.join(experiment_out_dir, "evaluation_results.csv"), index=False)
