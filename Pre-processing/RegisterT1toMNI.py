...
all_parameters = [
    'dtd_covariance_MD_to_t1_antstrans.nii.gz',
    'dtd_covariance_C_MD_clipped.nii.gz',
    'dtd_covariance_C_mu_to_t1_antstrans.nii.gz',
    'dtd_covariance_FA_to_t1_antstrans.nii.gz',
    'dtd_covariance_V_MD_to_t1_antstrans.nii.gz',
    'dtd_codivide_md_t_to_t1_antstrans.nii.gz',
    'dtd_codivide_v_at_to_t1_antstrans.nii.gz',
    'dtd_codivide_v_fw_to_t1_antstrans.nii.gz',
    't2_to_t1.nii.gz',
    'T1.nii.gz'
]

# === Get list of subject folders ===
ListofSubjectNames = [d for d in os.listdir(DMRIFolder)
                      if os.path.isdir(os.path.join(DMRIFolder, d)) and ('PD20' in d or 'PD_20' in d)]
...
for subj_path in ListofSubjectNames:
    ...
    subj_dmri = os.path.join(DMRIFolder, subj_name)
    subj_t1 = os.path.join(T1Folder, subj_name, "mri")
    subj_t2 = os.path.join(T2Folder, subj_name)
    ...
    for param_file in all_parameters:
        # Determine the correct input path depending on the file origin
        if param_file.startswith("t2"):
            param_path = os.path.join(subj_t2, param_file)
        elif param_file.startswith("T1"):
            param_path = os.path.join(subj_t1, "T1.nii.gz")
        else:
            param_path = os.path.join(subj_dmri, param_file)
        ...
        try:
            ...
            print(f"Error transforming {param_file}: {e}")
